/////////////////////////////////////////////////////////
//  ALGORITHM JAVASCRIPT
/////////////////////////////////////////////////////////

// findByIdAndUpdate(id, updateObject, arr)

// Given an id, an object that has keys with corresponding updated values, and an array of objects

// Find the object by "id" key that matches the given id value and then update that object's
// keys with the provided new values.

// Return the updated object, or null if no object was found

// check 
// someObj.hasOwnProperty("key")

// Object.keys(someObj) // returns [an array of "keys"]


const students = [
    {
        id: 1,
        name: "student1",
        isLateToday: false,
        lateCount: 15,
        redBeltStatus: false
    },
    {
        id: 2,
        name: "student2",
        isLateToday: false,
        lateCount: 1,
        redBeltStatus: false
    },
    {
        id: 3,
        name: "student3",
        isLateToday: false,
        lateCount: 0,
        redBeltStatus: false
    }
];

// Collection: Array of Objects
// Updated: Object with key value pairs
function findByIdAndUpdate(id, updatedVals, collection) {
    // Pseudo code:
    // 1 Go through collection to find object that matches id
    // 2 go through updated Vals, and update each new key value pair inside of found object in collection?
    // 3 Note: if key in updated Vals is not in student from collection

    const copyCollection = [...collection];

    for (let student of copyCollection) {
        if (student.id === id) {
            const arrayOfKeys = Object.keys(updatedVals);
            for (key of arrayOfKeys) {
                const valueFromKey = updatedVals[key];

                if (student.hasOwnProperty(key)) {
                    // Updated Key Value
                    student[key] = valueFromKey;
                }
            }
            return student;
        }
    }
    return null;

}

// --- TEST the following: ---
// Input: 3, { redBeltStatus: true }, students
// Output: {
//   id: 3,
//   name: "student3",
//   isLateToday: false,
//   lateCount: 0,
//   redBeltStatus: true
// }

console.log(
    findByIdAndUpdate(3, { redBeltStatus: true }, students)
);

// Input: 1, { isLateToday: true, lateCount: 16, randomKey: "randomValue"  }, students
// Output: {
//   id: 1,
//   name: "student1",
//   isLateToday: true,
//   lateCount: 16,
//   redBeltStatus: false
// }

console.log("\n*******************************");
console.log(
    findByIdAndUpdate(1, { isLateToday: true, lateCount: 16, randomKey: "randomValue"  }, students)
);

// Input: 5, {}, students
// Output: null

console.log("\n*******************************");
console.log(
    findByIdAndUpdate(5, {}, students)
);